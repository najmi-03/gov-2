              ...
              <div className="flex-1 overflow-y-auto custom-scrollbar p-5 md:p-8">
                ...
                <div className="flex border-b border-white/10 mb-6 md:mb-8 overflow-x-auto whitespace-nowrap scrollbar-hide">
                    ...
                    <>
                        ...
                        <button onClick={() => setActiveTab('carousel_mgmt')} className={`flex-shrink-0 px-4 py-3 text-[9px] font-bold tracking-widest uppercase transition-colors ${activeTab === 'carousel_mgmt' ? 'text-amber-500 border-b-2 border-amber-500' : 'text-slate-500 hover:text-white'}`}>Slider Carousel</button>
                ...
                <div className="pb-20">
                    ...
                    {activeTab === 'carousel_mgmt' && (
                        <div className="space-y-6">
                            ...
                            <div id="carousel-form-top" className={`border p-5 rounded-2xl space-y-4 transition-all ${editingSlideId ? 'bg-amber-500/5 border-amber-500/30' : 'bg-slate-950 border-white/5'}`}>
                                ...
                                <div className="space-y-2">
                                    <label className="text-[9px] font-bold text-slate-500 uppercase">Judul Utama</label>
                                    <input type="text" placeholder="Contoh: Visi Masa Depan" value={newSlideTitle} onChange={(e) => setNewSlideTitle(e.target.value)} className="w-full bg-slate-900 border border-white/10 rounded-lg px-4 py-3 text-sm text-white focus:border-amber-500/50 outline-none" />
                                </div>
                                <div className="space-y-2">
                            ...
                            <div className="space-y-4">
                                {carouselSlides.map((slide, i) => (
                                    <div key={slide.id} className="bg-slate-900 border border-white/5 p-4 rounded-xl flex gap-4 items-center group">
                                        <img src={slide.imageUrl} alt="" className="w-20 h-12 object-cover rounded-lg border border-white/10" />
                    ...
                    {/* ... (Existing tabs for inventory, recruitment, structural, salary, form_mgmt, legislative, terms) ... */}
