...
const NewsAdmin: React.FC<NewsAdminProps> = ({ 
  ...
  recruitmentLink = "", setRecruitmentLink, forms, setForms
}) => {
  ...
  // LOGIC SYNC: Ambil nama Presiden dari Database Struktural (Leadership state) jika role adalah Super Admin
  ...
  useEffect(() => {
    const loadCloudRecruitment = async () => {
       const saved = localStorage.getItem('ls_gov_recruitment_config');
       if (saved) setRecruitmentConfig(JSON.parse(saved));
       const cloudConfig = await fetchFromDatabase('RECRUITMENT');
       if (cloudConfig) {
         setRecruitmentConfig(cloudConfig);
         localStorage.setItem('ls_gov_recruitment_config', JSON.stringify(cloudConfig));
       }
    };
    loadCloudRecruitment();
  ...
  // Hapus akses jika role adalah NONE
  ...
  const handleSaveStructural = async () => {
      setIsSaving(true);
      localStorage.setItem('ls_gov_depts', JSON.stringify(depts));
      localStorage.setItem('ls_gov_leadership', JSON.stringify(leadership));
  ...
  const saveRecruitmentConfig = async (config: RecruitmentConfig) => {
    ...
    alert("Konfigurasi Rekrutmen Tersimpan ke Database!");
  ...
  // Preset Navigasi untuk Presiden
  ...
  return (
    <>
      <button onClick={() => setIsOpen(true)} className="fixed bottom-6 left-6 z-50 w-12 h-12 md:w-14 md:h-14 bg-amber-500 text-slate-950 rounded-full shadow-2xl flex items-center justify-center hover:scale-110 transition-transform">
            ...
            <motion.div initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} className="relative w-full md:max-w-2xl lg:max-w-3xl h-full bg-slate-900 border-l border-white/10 shadow-2xl p-5 md:p-8 overflow-y-auto custom-scrollbar">
              ...
              {isSuperAdmin && (
                <div className="relative mb-8 bg-amber-500/5 border border-amber-500/20 rounded-2xl p-4">
                    <div className="flex justify-between items-center cursor-pointer" onClick={() => setShowPresidentSwitch(!showPresidentSwitch)}>
                        ...
                        <button className="bg-amber-500 text-slate-950 px-3 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-widest flex items-center gap-2">
                            ...
                            <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="grid grid-cols-2 md:grid-cols-4 gap-2 mt-4 pt-4 border-t border-amber-500/10">
                                {presidentTabs.map(pt => (
                                    <button 
                                        ...
                                        className={`flex flex-col items-center justify-center p-3 rounded-xl border transition-all ${activeTab === pt.tab ? 'bg-amber-500 border-amber-500 text-slate-950' : 'bg-slate-950 border-white/5 text-slate-400 hover:border-amber-500/50 hover:text-white'}`}
                                    >
              ...
              <div className="flex border-b border-white/10 mb-6 md:mb-8 overflow-x-auto whitespace-nowrap scrollbar-hide">
                ...
                {(isSuperAdmin || userRole === 'PAWN_ADMIN' || userRole === 'PAWN_STAFF' || userRole === 'STAFF') && (
                  <button onClick={() => setActiveTab('inventory')} className={`flex-shrink-0 px-4 py-3 text-[9px] font-bold tracking-widest uppercase ${activeTab === 'inventory' ? 'text-amber-500 border-b-2 border-amber-500' : 'text-slate-500'}`}>
                ...
                {(isSuperAdmin || userRole === 'TREASURY_ADMIN') && <button onClick={() => setActiveTab('salary')} className={`flex-shrink-0 px-4 py-3 text-[9px] font-bold tracking-widest uppercase ${activeTab === 'salary' ? 'text-amber-500 border-b-2 border-amber-500' : 'text-slate-500'}`}>Penggajian</button>}
              ...
              {activeTab === 'news' && (
                  <div className="space-y-6">
                    <div className="bg-slate-950 border border-white/5 p-5 rounded-2xl space-y-4">
                      ...
                      <input type="text" placeholder="Judul Berita" value={newNewsTitle} onChange={(e) => setNewNewsTitle(e.target.value)} className="w-full bg-slate-900 border border-white/10 rounded-lg px-4 py-3 text-sm text-white focus:border-amber-500/50 outline-none" />
                      <textarea rows={4} placeholder="Isi Berita..." value={newNewsSummary} onChange={(e) => setNewNewsSummary(e.target.value)} className="w-full bg-slate-900 border border-white/10 rounded-lg px-4 py-3 text-sm text-white focus:border-amber-500/50 outline-none" />
                      <div className="flex gap-2">{['Umum', 'Ekonomi', 'Kesehatan', 'Hukum', 'Politik'].map(tag => (<button key={tag} onClick={() => setNewNewsTag(tag)} className={`px-3 py-1 rounded text-[10px] uppercase font-bold border ${newNewsTag === tag ? 'bg-amber-500 text-slate-950 border-amber-500' : 'bg-transparent text-slate-500 border-white/10'}`}>{tag}</button>))}</div>
                      <button onClick={handleAddNews} disabled={isSaving} className="w-full py-3 bg-amber-500 text-slate-950 font-bold rounded-xl uppercase tracking-widest text-xs shadow-lg shadow-amber-500/20">{isSaving ? 'Menyimpan...' : 'PUBLIKASIKAN BERITA'}</button>
                    </div>
                    <div className="space-y-4">{news.map((item) => (<div key={item.id} className="flex justify-between items-center bg-slate-900 p-4 rounded-xl border border-white/5"><div><h4 className="font-bold text-white text-sm">{item.title}</h4><p className="text-[10px] text-slate-500">{item.date}</p></div><button onClick={async () => { if(confirm("Hapus berita?")) { const updated = news.filter(n => n.id !== item.id); setNews(updated); await saveToDatabase('NEWS', updated); } }} className="text-red-500 hover:text-red-400">🗑️</button></div>))}</div>
                  </div>
              )}

              {activeTab === 'permission_portal' && <StaffPermissionPortal permissions={permissionConfig} staffName={currentDisplayName} />}
              ...
              {activeTab === 'feedback_config' && (
                <div className="space-y-6 pb-20">
                   ...
                   <div className="bg-slate-900 border border-white/10 rounded-2xl p-6 space-y-6">
                      ...
                      <button onClick={saveFeedbackConfig} className="w-full bg-amber-500 text-slate-950 font-bold py-3 rounded-xl uppercase tracking-widest text-xs hover:bg-amber-400">Simpan Konfigurasi</button>
                   </div>
                </div>
              )}
              {activeTab === 'permission_mgmt' && <PermissionManager permissions={permissionConfig} setPermissions={setPermissionConfig} />}
